...
//!
//! Manages metadata and content indexes for the skills directory.
...
///
/// Ensures skill_index and content_index are always consistent.
#[derive(Clone)]
...
impl SkillIndexer {
    ...
    ///
    /// Performs an atomic update of both indexes to ensure consistency.
    ...
    pub fn remove_skill(&self, name: &str) -> Result<(), IndexError> {
        ...
        debug!(
            "Removed skill {} ({} skills, {} content entries removed)",
    ...
    fn build_skill_index(&self) -> Result<SkillIndex, IndexError> {
        ...
        debug!(
            "Built skill index: {} skills, {} errors",
            skills.len(),
            errors.len()
        );
...
mod tests {
    ...
    fn test_incremental_update() {
        let temp_dir = TempDir::new().unwrap();
        create_test_skill(temp_dir.path(), "forms", "Form handling");
        ...
        // Add another skill.
        create_test_skill(temp_dir.path(), "auth", "Authentication patterns");
        indexer.update_skill("auth").unwrap();
        assert_eq!(indexer.get_skill_index().len(), 2);

        // Remove first skill.
        indexer.remove_skill("forms").unwrap();
        assert_eq!(indexer.get_skill_index().len(), 1);
        assert!(indexer.get_skill_meta("forms").is_none());
        assert!(indexer.get_skill_meta("auth").is_some());
    }

    #[test]
    fn test_skill_from_path() {
        let temp_dir = TempDir::new().unwrap();
        let indexer = SkillIndexer::new(temp_dir.path());

        let path = temp_dir.path().join("forms").join("SKILL.md");
        assert_eq!(indexer.skill_from_path(&path), Some("forms".to_string()));

        let hidden = temp_dir.path().join(".hidden").join("SKILL.md");
        assert_eq!(indexer.skill_from_path(&hidden), None);

        let outside = PathBuf::from("/somewhere/else");
        assert_eq!(indexer.skill_from_path(&outside), None);
    }
    ...
    fn test_path_traversal_blocked() {
        ...
        let result = validate_sub_skill_path(&skill_dir, "../../../etc/passwd");
        assert!(result.is_err());
    ...
    #[test]
    fn test_multiple_skills_indexed() {
        let temp_dir = TempDir::new().unwrap();
        create_test_skill(temp_dir.path(), "forms", "Form handling");
        create_test_skill(temp_dir.path(), "auth", "Authentication");
        create_test_skill(temp_dir.path(), "testing", "Testing patterns");

        let indexer = SkillIndexer::new(temp_dir.path());
        indexer.reload().unwrap();

        let skill_index = indexer.get_skill_index();
        assert_eq!(skill_index.len(), 3);

        let content_index = indexer.get_content_index();
        assert_eq!(content_index.len(), 3); // One SKILL.md each.
    }
}
