...
/// Validate skill metadata against the expected schema.
///
    ...
    if let Some(sub_skills) = &meta.sub_skills {
        ...
        for sub in sub_skills {
            if !seen_names.insert(&sub.name) {
                errors.push(format!("sub_skills: duplicate name '{}'", sub.name));
            }
        }
    }
...
pub fn validate_skills(
    ...
    for skill in &index.skills {
        let skill_dir = indexer.skills_dir().join(&skill.name);

        // Validate metadata.
        if let Err(errors) = validate_meta(skill) {
            for err in errors {
                result.add_error(format!("{}: {}", skill.name, err));
            }
        }
        ...
        // Warn about missing discoverability.
...
mod tests {
    ...
    fn test_invalid_sub_skill_file() {
        let meta = SkillMeta {
            ...
            sub_skills: Some(vec![SubSkillMeta {
                ...
                file: "react/SKILL.txt".to_string(),
                triggers: vec![],
            }]),
            source: None,
        };
    ...
    #[test]
    fn test_validate_skills_valid() {
        let temp_dir = tempfile::TempDir::new().unwrap();
        let skill_dir = temp_dir.path().join("forms");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("_meta.json"),
            r#"{"name": "forms", "description": "Form handling", "tags": ["validation"]}"#,
        )
        .unwrap();
        std::fs::write(skill_dir.join("SKILL.md"), "# Forms\n\nContent").unwrap();

        let indexer = crate::indexer::SkillIndexer::new(temp_dir.path());
    ...
    #[test]
    fn test_validate_skills_missing_skill_md() {
        let temp_dir = tempfile::TempDir::new().unwrap();
        let skill_dir = temp_dir.path().join("forms");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("_meta.json"),
            r#"{"name": "forms", "description": "Form handling"}"#,
        )
        .unwrap();
        // No SKILL.md
