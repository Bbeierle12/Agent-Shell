...
impl SandboxExecutor {
    ...
    fn docker_base_args(&self) -> (Vec<String>, String) {
        ...
        // --stop-timeout: grace period (seconds) for SIGTERM before SIGKILL.
        // Use a small value; the actual execution timeout is enforced by Tokio.
    ...
    async fn docker_run(&self, cmd: &[&str]) -> Result<ExecResult, AgentError> {
        ...
        // Capture PID so we can kill the process on timeout (wait_with_output consumes child).
        let pid = child.id();
        ...
        .await
        {
            ...
            Ok(Err(e)) => Err(AgentError::Sandbox(format!("Failed to run docker: {}", e))),
            Err(_) => {
                // Timeout: kill the docker process by PID to clean up.
                if let Some(pid) = pid {
                ...
                Err(AgentError::Sandbox("Docker execution timed out".into()))
            }
        }
    }
    ...
    async fn docker_run_stdin(&self, cmd: &[&str], input: &str) -> Result<ExecResult, AgentError> {
        let (mut docker_args, image) = self.docker_base_args();
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(input.as_bytes()).await.map_err(|e| {
                AgentError::Sandbox(format!("Failed to write to docker stdin: {}", e))
            })?;
...
/// Maximum output size (bytes) per stream (stdout/stderr) to prevent
/// tool results from flooding the model context window.
...
impl ExecResult {
    ...
    ///
    /// Stdout and stderr are individually capped at `MAX_OUTPUT_BYTES` to
    /// prevent large outputs from flooding the model's context window.
    pub fn to_display_string(&self) -> String {
        let mut parts = Vec::new();
        if !self.stdout.is_empty() {
            parts.push(format!(
                "stdout:\n{}",
                truncate_output(&self.stdout, MAX_OUTPUT_BYTES)
            ));
        }
        if !self.stderr.is_empty() {
            parts.push(format!(
                "stderr:\n{}",
                truncate_output(&self.stderr, MAX_OUTPUT_BYTES)
            ));
        }
